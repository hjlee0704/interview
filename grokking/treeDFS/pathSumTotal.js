...
const find_paths = function(root, sum) {
  ...
  while (stack.length > 0) {
    ...
    currentNode[2] = currentNode[0].value + currentNode[2];
    let subArr = [...currentNode[1]];
    subArr.push(currentNode[0].value);
    if (currentNode[2] === sum) {
      allPaths.push(subArr);
    }
    if (currentNode[0].left !== null) {
      stack.push([currentNode[0].left, subArr, currentNode[2]]);
    }
    if (currentNode[0].right !== null) {
      stack.push([currentNode[0].right, subArr, currentNode[2]]);
    }
